...
class BuyerFeedbackScreen extends StatefulWidget {
  const BuyerFeedbackScreen({Key? key}) : super(key: key);

  @override
  _BuyerFeedbackScreenState createState() => _BuyerFeedbackScreenState();
}

class _BuyerFeedbackScreenState extends State<BuyerFeedbackScreen> {
  ...
  void initState() {
    super.initState();
     // Listen for changes in the buyerFeedback collection
    firestore.collection('buyerFeedback').orderBy('timestamp', descending: false).snapshots().listen((event) {
      scrollToBottom();
    });
  }
  ...
  void sendFeedback() async {
    ...
    try {
      // Add the feedback to the buyerFeedback collection in Firestore
      await firestore.collection('buyerFeedback').add({
        'senderId': user.uid,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Buyer Feedback to Admin'),
          ...
          children: <Widget>[
            Expanded(
              child: StreamBuilder<QuerySnapshot>(
                stream: firestore.collection('buyerFeedback').orderBy('timestamp', descending: false).snapshots(),
